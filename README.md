...
**Note**: Adding custom icons via the fourth parameter in kodi-send may not work reliably on all systems, so the add-on uses text-only notifications for maximum compatibility.
...
```bash
show_notification() {
    kodi-send --action="Notification($1,$2,15000)"  # 15 seconds instead of 10
}
```
